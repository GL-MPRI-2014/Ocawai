let rec int_of_bytes string =
  match String.length string with
    | 0 -> assert false
    | 1 -> Char.code string.[0]
    | _ -> 256 * Char.code string.[0] + int_of_bytes (String.sub string 1 (String.length string))
      ...
      (fun client ->
        if client = fd_listen then
		  ...
		  let continue  = ref true in
		    ...
		    let length = String.sub data 0 1 in
                    let index = int_of_bytes length in
		    if (String.length data >= index) then
		      begin
			Hashtbl.replace input_buffer client
			  (String.sub data
			     (index + 1)
